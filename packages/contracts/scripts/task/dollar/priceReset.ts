...
const func = async (params: TaskFuncParam) => {
    ...
    const bondingAddr = await managerContract.bondingContractAddress();
    const bondingDeployments = await deployments(chainId.toString(), "Bonding");
    const bondingContract = new ethers.Contract(bondingAddr, bondingDeployments.abi, signer);
    ...
    const available_lp_amount = await metaPoolContract.balanceOf(bondingAddr);
    console.log({
    ...
    const tx = await bondingContract.uADPriceReset(burn_lp_amount);
